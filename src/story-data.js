const moods = ["feral", "neon", "gremlin", "static", "glitch", "wobbly", "snide", "dream-dumb"];
const transitions = ["tilt", "blink", "melt", "swerve", "shiver", "spin", "snap", "drift"];
const creatures = ["mango goblin", "yard oracle", "cursed pigeon", "desk goblin", "moon rat", "mood raccoon", "paper phantom", "keyboard shrimp"];
const verbs = ["haunts", "snickers at", "unfolds over", "jumps through", "throws crumbs at", "judges", "loops around", "spits glitter on"];
const objects = ["a red stapler", "a rubber crown", "a broken vending machine", "a haunted loafer", "a polite tornado", "a screaming salad", "a fake moon", "a suspicious banana"];
const endings = ["because dignity left the building", "and that is somehow the whole point", "while nobody was looking", "like a bad idea with excellent lighting", "to prove the floor can blink", "for no reason except vibes", "because the universe misclicked", "and now the walls are laughing"];
...
export const rabbitHoles = Array.from({ length: 100 }, (_, index) => {
  ...
  return {
    ...
    hook: `Tap to fall into a ${mood} rabbit hole with a ${transition} transition.`,
